...
const navItems = [
  ...
  ['For People', '#trust'], ['Impact', '#impact'], ['Insights', '#insights'], ['Contact', '#contact']
];

const problems = [
  { icon: UsersRound, title: 'Unclear Guidance', text: 'People deserve simple answers and a clear next step when they need care.' },
  { icon: Network, title: 'Fragmented Care', text: 'Care teams and services often work in isolation, making every step harder.' },
  ...
  { icon: UserRound, title: 'Too Much to Manage', text: 'Appointments, reports, payments, and continued care should feel effortless.' }
];

const solutions = [
  { icon: UsersRound, title: 'Care Access', text: 'Find the right doctor or specialist, anytime, anywhere.', accent: 'text-blue-500' },
  { icon: FlaskConical, title: 'Smart Diagnostics', text: 'Modern labs, imaging, and home sample collection made simple.', accent: 'text-violet-500' },
  ...
  { icon: BrainCircuit, title: 'Intelligent Infrastructure', text: 'Secure technology that helps every part of care work better for people.', accent: 'text-indigo-500' }
];
...
const trustCards = [
  ...
  { icon: Scale, title: 'Responsible Guidance', text: 'Intelligent insights supported by human judgment.' }
];

const impactStats = [
  { value: '1.8M+', label: 'Lives Supported', icon: HeartHandshake },
  { value: '35%', label: 'Lower Personal Healthcare Costs', icon: WalletCards },
    ...
    <section id="top" className="relative min-h-[760px] overflow-hidden bg-hero-radial pt-[76px]">
      <div className="mx-auto grid min-h-[680px] max-w-[1380px] items-center gap-10 px-5 py-16 lg:grid-cols-[.92fr_1.08fr] lg:px-8 lg:py-20">
        <div className="hero-copy relative z-20 max-w-2xl">
          <span className="inline-flex rounded-full border border-gold-400 bg-white/90 px-4 py-2 text-[11px] font-extrabold tracking-[0.13em] text-gold-500">THE FUTURE OF HEALTHCARE IS HERE</span>
          <h1 className="mt-6 text-[2.65rem] font-extrabold leading-[1.06] tracking-[-0.035em] text-navy-900 sm:text-6xl lg:text-[4.45rem]">Healthcare that evolves around people.</h1>
          <p className="mt-6 max-w-xl text-base leading-8 text-slate-600 sm:text-lg">Astikan brings intelligent care, smarter diagnostics, digital health services, and simpler payments into everyday life. Our technology helps people receive faster answers, easier support, and care that feels personal.</p>
          <div className="mt-8 flex flex-wrap gap-4">
            ...
            <a href="#technology" className="inline-flex items-center gap-2 rounded-md border border-navy-700 bg-white px-6 py-3.5 text-sm font-bold text-navy-900 transition hover:bg-slate-50">Explore Our Technology <ArrowRight size={16}/></a>
          </div>
          <div className="mt-10 grid max-w-2xl grid-cols-2 gap-5 border-t border-slate-200 pt-7 sm:grid-cols-4">
            {[[Globe2,'25+','Countries'],[Hospital,'10,000+','Partner Facilities'],[UsersRound,'1.8M+','Lives Supported'],[ShieldCheck,'99.6%','Platform Reliability']].map(([Icon, value, label]) => <div key={label} className="flex items-center gap-3"><div className="text-blue-500"><Icon size={25} strokeWidth={1.6}/></div><div><div className="text-lg font-extrabold text-navy-900">{value}</div><div className="text-[11px] text-slate-500">{label}</div></div></div>)}
          </div>
        </div>
        <div className="hero-visual relative flex min-h-[520px] items-center justify-center lg:min-h-[610px]">
          <img src={bannerImage} alt="Astikan healthcare services" className="h-auto w-full object-contain" />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="border-t border-slate-100 bg-white py-16 sm:py-20">
      <div className="mx-auto max-w-[1380px] px-5 lg:px-8">
        <SectionTitle kicker="A Better Way Forward" title="Healthcare should feel easier for everyone." />
...
function SolutionsSection() {
  return (
    <section className="bg-slate-50/70 py-16 sm:py-20">
      <div className="mx-auto max-w-[1380px] px-5 lg:px-8">
        <SectionTitle kicker="Designed for Tomorrow" title="Intelligent care. Made deeply human." />
    ...
    <section id="ecosystem" className="bg-white py-16 sm:py-24">
      <div className="mx-auto grid max-w-[1380px] gap-12 px-5 lg:grid-cols-[.56fr_1.44fr] lg:px-8">
        <div data-aos="fade-right">
          <SectionTitle kicker="Healthcare in Motion" title="Every step of care, designed around you." center={false} />
          <p className="mt-5 max-w-md text-base leading-7 text-slate-600">Astikan helps doctors, hospitals, diagnostics, payments, and digital services work in harmony so people can move through care with confidence and ease.</p>
  ...
  return (
    <section id="technology" className="relative overflow-hidden bg-dark-grid py-16 sm:py-24">
      ...
      <div className="mx-auto grid max-w-[1380px] items-center gap-12 px-5 lg:grid-cols-[.55fr_1.45fr] lg:px-8">
        <div data-aos="fade-right"><SectionTitle kicker="Technology for Life" title="Intelligence that makes care feel human." center={false} light/><p className="mt-5 text-base leading-7 text-blue-100/80">Our platform turns complex health information into clear actions, giving people and care teams more time for what matters most.</p><div className="mt-7 space-y-3">{['Care updates when they matter','One clear health record','Helpful personal guidance','Simple payments and claims','Technology that works across care'].map(x=><div key={x} className="flex items-center gap-3 text-sm text-white"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-white/10"><Check size={12}/></span>{x}</div>)}</div></div>
        <TechDashboard />
      </div>
    </section>
  );
    ...
    <section id="trust" className="bg-white py-16 sm:py-20">
      <div className="mx-auto max-w-[1380px] px-5 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[.33fr_1.67fr]">
          <SectionTitle kicker="Built for People" title="Technology with care at its heart." center={false}/>
...
function CTA() {
  return (
    <section id="contact" className="relative overflow-hidden bg-gradient-to-r from-navy-800 via-navy-900 to-[#08305f] py-8">
      ...
      <div className="relative mx-auto flex max-w-[1180px] flex-col items-center justify-between gap-6 px-5 text-center md:flex-row md:text-left"><div><h3 className="text-xl font-extrabold text-white">Step into a more human future of healthcare.</h3><p className="mt-1 text-sm text-blue-100/80">Astikan is here to make every step simpler, smarter, and more personal.</p></div><div className="flex flex-wrap justify-center gap-4"><a href="mailto:care@astikan.health" className="inline-flex items-center gap-2 rounded-md bg-white px-7 py-3.5 text-sm font-extrabold text-navy-900">Start Care Journey <ArrowRight size={16}/></a><a href="tel:+910000000000" className="inline-flex items-center gap-2 rounded-md border border-white/70 px-7 py-3.5 text-sm font-extrabold text-white">Talk to Our Care Team <ArrowRight size={16}/></a></div></div>
    </section>
  );
}
  ...
  return (
    <footer id="insights" className="bg-[#031a3b] pt-12 text-white">
      <div className="mx-auto grid max-w-[1380px] gap-10 px-5 lg:grid-cols-[1.25fr_3.75fr] lg:px-8">
        <div><AstikanLogo light/><p className="mt-5 max-w-xs text-sm leading-6 text-blue-100/70">Astikan represents the future of healthcare. We bring intelligent technology and human care together to help people live healthier lives.</p><div className="mt-6 flex gap-3">{[[FaLinkedinIn,'LinkedIn'],[FaXTwitter,'X'],[FaYoutube,'YouTube'],[FaInstagram,'Instagram']].map(([Icon,label])=><a key={label} href="#" aria-label={label} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/20 text-blue-100 transition hover:bg-white hover:text-navy-900"><Icon size={15}/></a>)}</div></div>
      ...
      <div className="mx-auto mt-10 flex max-w-[1380px] flex-col gap-3 border-t border-white/10 px-5 py-5 text-[11px] text-blue-100/55 sm:flex-row sm:items-center sm:justify-between lg:px-8"><span>© 2026 Astikan Healthcare. All rights reserved.</span><span>Creating a healthier future for everyone. 💙</span></div>
    </footer>
  );
}
