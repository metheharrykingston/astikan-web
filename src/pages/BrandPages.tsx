...
export function CompanyPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-700">
      <SiteHeader />
      <main>
        <section className="relative overflow-hidden bg-[linear-gradient(180deg,#ffffff,#f5f8ff)] px-5 pb-16 pt-[120px] lg:px-8 lg:pb-24 lg:pt-[148px]"><div className="mx-auto grid max-w-[1380px] items-center gap-12 lg:grid-cols-[.8fr_1.2fr]"><div data-aos="fade-right"><p className="text-xs font-extrabold uppercase tracking-[.22em] text-blue-600">ABOUT ASTIKAN</p><h1 className="mt-6 text-5xl font-extrabold leading-[.98] tracking-[-0.055em] text-navy-900 sm:text-7xl">Creating a <span className="text-blue-600">more human future for healthcare.</span></h1><p className="mt-7 max-w-xl text-base leading-8 text-slate-600 sm:text-lg">Astikan creates digital and physical healthcare products that turn advanced technology into clear, helpful experiences for people everywhere.</p><div className="mt-9 grid gap-4 sm:grid-cols-3">{[[UsersRound,'People first'],[ShieldCheck,'Technology driven'],[HandHeart,'Purpose led']].map(([Icon,title])=><div key={title} className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600"><Icon size={18} /></span><div><div className="text-xs font-extrabold text-navy-900">{title}</div><div className="mt-1 text-[10px] leading-5 text-slate-500">Built around real needs.</div></div></div>)}</div></div><CompanyVisual /></div></section>
