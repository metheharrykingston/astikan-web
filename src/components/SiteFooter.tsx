        ...
        <div className="mx-auto max-w-[1380px]">
          <div className="grid items-end gap-8 lg:grid-cols-[1fr_.72fr]">
            <div data-aos="fade-up">
              ...
              <h2 className="mt-4 max-w-4xl text-4xl font-extrabold tracking-[-0.035em] text-navy-900 sm:text-5xl lg:text-6xl">A growing family of health technologies.</h2>
            </div>
            <p className="max-w-xl text-base leading-8 text-slate-600 lg:justify-self-end" data-aos="fade-up" data-aos-delay="90">Astikan is building connected digital and physical systems that make health experiences clearer, calmer, and more useful across everyday life.</p>
          </div>
          ...
          <div className="mt-14 grid gap-5 lg:grid-cols-3">
            {worldCards.map(({ eyebrow, title, text, to, icon: Icon, enabled }, index) => {
              const content = (
                <>
                  <div className="flex items-start justify-between gap-6">
                    <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-700"><Icon size={26} strokeWidth={1.45} /></span>
                    <span className={`rounded-full px-3 py-1 text-[9px] font-extrabold uppercase tracking-[0.14em] ${enabled ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>{enabled ? 'Explore' : 'In development'}</span>
                  </div>
              ...
              return enabled ? <a key={title} href={to} className={className} data-aos="fade-up" data-aos-delay={index * 70}>{content}</a> : <article key={title} className={className} data-aos="fade-up" data-aos-delay={index * 70}>{content}</article>;
            })}
          </div>
        </div>
      </section>
          ...
          <div className="grid gap-14 lg:grid-cols-[1.45fr_1fr_1fr_1fr]">
            ...
            {footerGroups.map((group, groupIndex) => (
              <div key={group.title} data-aos="fade-up" data-aos-delay={(groupIndex + 1) * 65}>
                ...
                <div className="mt-6 space-y-4">{group.items.map((item) => <FooterLink key={item.label} item={item} />)}</div>
              </div>
            ))}
          </div>

          <div className="mt-16 grid gap-5 border-y border-white/10 py-8 sm:grid-cols-3" data-aos="fade-up">
            {[[Orbit, 'Connected systems', 'Digital and physical health technologies designed as one growing world.'], [FlaskConical, 'Long-term research', 'Research-led exploration of what health technology can become next.'], [Atom, 'Built for life', 'A mission extending beyond individual products toward life wherever it exists.']].map(([Icon, title, text]) => <div key={title} className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white/[.06] text-cyan-300"><Icon size={20} strokeWidth={1.45} /></span><div><div className="text-sm font-extrabold text-white">{title}</div><div className="mt-2 text-xs leading-5 text-blue-100/50">{text}</div></div></div>)}
          </div>
